...
// summarizeFindings parses the flat findings YAML emitted by knowledge.UseCase
// (a root key followed by "- name:/domain:/evidence:/source:" list items) and
...
func codexNoKnowledgeSection() string {
	...
	b.WriteString("Run `bqa discover`, `bqa ingest2`, then `bqa build` to extract project-specific QA knowledge, ")
